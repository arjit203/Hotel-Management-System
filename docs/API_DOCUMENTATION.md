...
**Past dates (2026-09-25 d):** a check-in before today (the property's calendar day, Asia/Kolkata) is refused with `400` and `errors: [{ field: "checkInDate", message: "Check-in date cannot be in the past." }]` — `field: "checkIn"` on the availability endpoint. Today is accepted. Previously both endpoints accepted any past date. Send dates as `YYYY-MM-DD`.
